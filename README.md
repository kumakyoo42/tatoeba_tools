...
All programs can deal with `sentences.csv` as well as with
...
Can be used with translation files generated by `add_translation.php`
too.
...
Adds translations to a file with sentences. The translations are added
as separate lines starting with two spaces. In addition it writes a
file with all sentences that do not have a translation.

For example, for adding german translations to greek sentences I'm
using
`php add_tranlation.php greek.csv german.csv links.csv greek_with_translation.csv greek_without_translation.csv`.

`greek.csv` and `german.csv` contain the greek and german sentences,
`links.csv` is taken directly from the Tatoeba download page. The
program writes output to two files: `greek_with_translation.csv` and
`greek_without_translation.csv`.

Usage:

```
php add_translation.php inputfile langfile linkfile withfile withoutfile
```
